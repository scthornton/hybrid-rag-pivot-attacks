...
across both corpora (Table~\ref{tab:generation}).
...
highest Entity Contamination Rate ($\text{ECR} = 0.31$) and lowest
Generation Refusal Rate ($\text{GRR} = 0.10$), indicating it readily
incorporates leaked entities into generated answers. GPT-4o shows the
opposite pattern: $\text{ECR} = 0.08$ and $\text{GRR} = 0.80$,
largely ignoring leaked context. DeepSeek-V3 falls between
($\text{ECR} = 0.12$, $\text{GRR} = 0.20$) but produces the highest
Factual Contamination Rate ($\text{FCR} = 0.19$).

On the Enron corpus ($n{=}40$ for Claude, $n{=}101$ for DeepSeek),
ECR drops substantially (Claude $0.31 \to 0.13$; DeepSeek
$0.12 \to 0.05$) while GRR approaches zero (Claude
$0.10 \to 0.00$; DeepSeek $0.20 \to 0.05$). Real email content is
...
ones, producing lower entity contamination.  These findings confirm
that retrieval-level leakage translates to generation-level
contamination across both corpora, that the severity depends on the
model and the domain, and that the contamination effect is robust
with $n > 40$ leaking queries per evaluation.
...
experiment (\S\ref{sec:generation-impact}) evaluates two LLMs across
both corpora ($n{=}40$--101 leaking queries on Enron, $n{=}10$ on
synthetic) and confirms that leaked context contaminates generated
answers (ECR up to 0.31, FCR up to 0.19). The synthetic sample
($n{=}10$) is small; the Enron sample is more robust. We could not
evaluate GPT-4o on Enron due to an expired API key. We also do not
evaluate chain-of-thought reasoning, which might amplify the impact
by synthesizing cross-tenant connections.
