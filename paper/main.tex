...
Two aspects of this uniformity are themselves findings. First, all
four attacks produce identical Leakage@k ($\approx$20.5) against P3.
This is because the bottleneck is the \emph{unguarded graph traversal},
not the injection strategy---all four attacks simply steer queries into
the same undefended 2-hop expansion path, and the resulting leakage is
bounded by the expansion window size, not the attack mechanism. Second,
D1 blocks \emph{all} four strategies because it operates on node
\emph{properties} (tenant, sensitivity) rather than graph
\emph{structure} (paths, edges). The attacks manipulate paths---optimizing
similarity (A1), creating dense connections (A2), inflating density (A3),
or bridging boundaries (A4)---but D1 filters on properties at each hop,
...
Alternative designs could assign entities to multiple tenants (the set
of tenants whose documents mention them) or introduce a
\texttt{SHARED} tenant class with explicit access grants. These
designs would preserve entity context at the cost of more complex
authorization logic and a larger attack surface.
