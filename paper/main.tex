...
This amplification occurs organically---no adversarial injection is
required. Shared entities (vendors, infrastructure, compliance
standards) create natural cross-tenant pivot paths in any multi-tenant
knowledge graph. We propose five layered defenses and show that per-hop
authorization alone eliminates all measured leakage
($\text{RPR} \to 0.0$) across all query types and attack variants.
The defense is deliberately simple: it re-checks existing tenant and
sensitivity metadata at the graph expansion boundary. That a trivial
check suffices is not a limitation---it reveals that the vulnerability
is a boundary placement problem, not a defense complexity problem.
\end{abstract}
...
security boundary that has not been systematically measured or
formalized in prior work. Consider an analyst
...
\begin{enumerate}
\item \textbf{Vulnerability characterization.} We identify and
  formalize a \emph{compound attack surface} that emerges from combining
  two individually secure retrieval modalities. Vector retrieval with
  tenant prefiltering is secure. Graph retrieval within a single
  tenant's subgraph is secure. But connecting them---using vector
  results as graph traversal seeds---creates $160\text{--}194\times$
  amplified leakage that neither modality exhibits alone. We define
  Retrieval Pivot Risk (RPR) and companion metrics (AF, PD,
  Leakage@k, severity-weighted leakage) that quantify the severity
  of this composition vulnerability (\S\ref{sec:metrics}).

\item \textbf{Attack taxonomy and organic leakage.} We present four
  Retrieval Pivot Attacks (A1--A4) that exploit the pivot boundary
  with injection budgets of 10--20 chunks. Critically, we also
  demonstrate that \emph{no adversarial injection is required}: organic
  shared entities (vendors, infrastructure, compliance standards) create
  natural cross-tenant pivot paths that produce RPR $= 0.95$ on benign
  queries (\S\ref{sec:attacks}, \S\ref{sec:results}).

\item \textbf{Defense placement analysis.} We design and evaluate five
  layered defenses (D1--D5) and show that the critical insight is not
  the defense mechanism itself but its \emph{placement}: authorization
  must be re-checked at the graph expansion boundary. The deliberate
  simplicity of D1 (per-hop authorization) is evidence that the
  vulnerability is a \emph{boundary placement problem}, not a
  \emph{defense complexity problem}. Existing metadata (tenant labels,
  sensitivity tiers) is sufficient---the missing element in production
  systems is checking it at the right architectural layer. The
  remaining defenses (D2--D5) serve as utility optimizers and
  defense-in-depth layers. A 27-configuration traversal sweep
  identifies total node budget as the primary leakage-controlling
  parameter (\S\ref{sec:mitigations}, \S\ref{sec:results}).
\end{enumerate}
...
The defense is deliberately simple: it re-uses existing graph metadata
with negligible latency ($<$1ms). That a trivial check suffices is
not a weakness of the defense---it is evidence that production hybrid
RAG systems are missing a critical authorization check at the
vector-to-graph boundary, and that adding it is both necessary and
straightforward.
